...
@bot.tree.command(name="deletequota", description="Delete the most recent logged shift (Lt. Colonel+ only)", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(user="The user whose most recent shift you want to delete")
async def deletequota(interaction: discord.Interaction, user: discord.Member):
    if not has_permission_for_others(interaction.user):
        await interaction.response.send_message("\u274c You don't have permission to delete quota entries.", ephemeral=True)
        return

    c.execute("SELECT rowid FROM shifts WHERE user_id = ? ORDER BY rowid DESC LIMIT 1", (str(user.id),))
    result = c.fetchone()

    if result:
        c.execute("DELETE FROM shifts WHERE rowid = ?", (result[0],))
        conn.commit()
        await interaction.response.send_message(f"\u2705 Successfully deleted the most recent shift logged for {user.mention}.", ephemeral=True)
    else:
        await interaction.response.send_message("\u26a0\ufe0f No logged shifts found for this user.", ephemeral=True)
