...
# DELETE QUOTA command: deletes the most recent shift log for a specified user
@bot.tree.command(name="deletequota", description="Delete the most recent shift log for a user", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(user="The user whose latest quota you want to delete")
async def deletequota(interaction: discord.Interaction, user: discord.Member):
    ...
    if row:
        c.execute("DELETE FROM shifts WHERE rowid = ?", (row[0],))
        conn.commit()
        await interaction.response.send_message(f"✅ Deleted the most recent shift log for {user.mention}.", ephemeral=False)
    else:
        await interaction.response.send_message("⚠️ No shift logs found for that user.", ephemeral=True)
